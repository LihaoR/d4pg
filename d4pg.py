...
class DDPG(object):
    ...
    def learn(self):
        ...
        #print 'br', br
        q_ = sess.run(self.q_, feed_dict={self.s_: bs_})
        #print 'q_',q_
        #print q_[0]
        q_target_batch = []
        for i in range(len(q_)):
            ...
            for j in range(len(qi)):
                z_target_step.append(GAMMA * qi[j] + br[i][0])
            #print z_target_step
        ...
        sess.run([self.update_c_op], feed_dict={self.s: bs, self.a: ba, self.q_target:q_target_batch})
    ...
    def work(self):
        ...
        for i in range(MAX_EPISODES):
            ...
            for j in range(MAX_EP_STEPS):
                ...
                ep_reward += r
                if j == MAX_EP_STEPS-1:
                    print('name', self.name, 'Episode:', i, ' Reward: %i' % int(ep_reward), 'Explore: %.2f' % var, )
                    break
